...
def resnet50(num_classes,
             ...
             branch=None,
             use_final_features_interaction=False):
  ...
  if use_final_features_interaction:
    features = tf.expand_dims(x, axis=1)
    features_T = tf.transpose(features, perm=[0, 2, 1])
    feature_squares = tf.matmul(features_T, features)
    features_upper = tf.linalg.band_part(feature_squares, 0, -1)
    ones = tf.ones(features_upper.shape[1:])
    mask = tf.linalg.band_part(ones, 0, -1)
    feature_interactions = tf.boolean_mask(features_upper, mask, axis=1)
    x = feature_interactions
    logging.info('@Final feature interactions ON ({})'.format(x))
